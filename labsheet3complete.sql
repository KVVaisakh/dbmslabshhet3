create table customeromer
(
customeromerid varchar(3),
Lname varchar(15),
Fname varchar(15),
area varchar(2),
phoneno numeric(8,0),
primary key(customeromerid));
create table movie
(
mvno numeric(2,0),
title varchar(25),
...
insert into customer(customerid,Lname,Fname,area,phoneno)
...
insert into invoice(invno,mvno,customerid,issueDate,retDate)
values
...
select * from invoice;
/*1*/
select concat(Fname,' ',Lname) as "Name" from customer;
/*2*/
select * from customer;
/*3*/
...
/*24 ?*/
select Fname,title from customer,movie,invoice
where invoice.customerid=customer.customerid and movie.mvno=invoice.mvno;
/*25*/
select customerid,Fname,Lname,area from customer
where phoneno is null;
/*26*/
delete from invoice
...
/*27*/
update customer
set area='vs' where customerid='A05';
/*28*/
select customerid,mvno from invoice;
/*29*/
select customerid,to_char(retDate, 'Month') as "Month"
from invoice;
/*30*/
select * from movie,invoice
where invoice.mvno=movie.mvno and movie.star='Amir';
/*31 */
select title from customer,movie,invoice
where invoice.mvno=movie.mvno and invoice.customerid=customer.customerid and customer.Fname='Vandana';
/*32*/
alter table invoice
...
set remark='Good';
/*33*/
select mvno from invoice,customer
where customer.customerid=invoice.customerid and customer.Fname='Pramada';
/*34*/
update customer
set phoneno=466389 where Fname='Pramada';
/*35*/
update invoice
set issuedate='24-jul-1993' where customerid='A01';
/*36*/
update movie
